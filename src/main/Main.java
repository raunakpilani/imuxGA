...
//		for (int connProb = 1; connProb < 100; connProb++) {
			...
			// initially read two random vertices from disconnVerts and add a link
			// between them
			...
			graphFileWriter.write(n1 + " " + n2 + " " + len * 10 + " " + capacity);
			...
			for (int i = 0; i < vertNum; i++) {
				for (int j = 1; j < vertNum; j++) {
					...
					if (!existingEdges.containsKey(edge)
						...
						len = rnd.nextInt(maxLenBy10);
						graphFileWriter.write(i + " " + j + " " + len * 10 + " " + capacity);
					}
				}
			}
		//}
